...
(function(){
    var cards = [
        { from: 'Oryol', to: 'Tula', vehicle: 'train', flight_number: '123', place_number: '17', baggage_number: '', field: 'example' },
        { from: 'Voronezh', to: 'Kursk', vehicle: 'bus', flight_number: '41', place_number: '', baggage_number: '001' },
        ...
        { from: 'Krasnodar', to: 'Rostov-na-Donu', vehicle: 'train', flight_number: '45A', place_number: '26', baggage_number: '' },
        { from: 'Tula', to: 'Moscow', vehicle: 'bus', flight_number: '36', place_number: '', baggage_number: '', Empty: '' },
    ...
    function build(cards){
        ...
        try{
            ...
            sort(cards).forEach((card) => { //Сортировка и последующий перебор массива для построения DOM модели
    ...
    function generatePhrase(card){
        ...
        phrase += card.place_number?
            phrases[rand(0, phrases.length)]:
            `No seat assignment. `;
        phrase += card.baggage_number?
            `Baggage drop at ticket counter ${card.baggage_number}. `:
            `Baggage will be automatically transferred from your last leg. `;

        for(var attr in card){  //Если есть не стандартные поля выводим их в виде 'свойство: значение'
